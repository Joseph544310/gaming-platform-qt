...
Cabo::Cabo(User *user, int gID)
{
    ...
    cat_img->setPixmap((QPixmap("images/cabo/animals/cat.png")).scaled(120,120));
    cat_img->setPos(960,300);
    this->addItem(cat_img);

    dog_img = new QGraphicsPixmapItem();
    dog_img->setPixmap((QPixmap("images/cabo/animals/dog.png")).scaled(120,120));
    dog_img->setPos(230,300);
    this->addItem(dog_img);

    wolf_img = new QGraphicsPixmapItem();
    wolf_img->setPixmap((QPixmap("images/cabo/animals/wolf.png")).scaled(120,120));
    wolf_img->setPos(610,30);
    this->addItem(wolf_img);

    player_img = new QGraphicsPixmapItem();
    if(user->gender == "f") {
        player_img->setPixmap((QPixmap("images/cabo/animals/diane.png")).scaled(120,120));
    } else {
        player_img->setPixmap((QPixmap("images/cabo/animals/morty_face.png")).scaled(120,120));
    }
    player_img->setPos(610,650);
    this->addItem(player_img);

    card_back_img = new QGraphicsPixmapItem();
    card_back_img->setPixmap((QPixmap("images/cabo/cards/card_back_background.png")).scaled(80,105));
...
void Cabo::place(int plrnum, CaboCard * card, int pos) {
    ...
    if(card->faceup) {
        card->setPixmap((QPixmap(QString("images/cabo/cards/card%1.png").arg(card->number))).scaled(80,105));
    } else {
        card->setPixmap((QPixmap(QString("images/cabo/cards/card_back_background.png"))).scaled(80,105));
    }
    card->setPos(x, y);
    card->show();
    this->addItem(card);
...
void Cabo::calledCabo() {
    ...
    if(user->username != "Guest") {
        QString val;
        QFile file;
        file.setFileName("users/users.json");
